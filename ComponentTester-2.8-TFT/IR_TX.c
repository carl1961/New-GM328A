...
void IR_Send_Code(uint8_t Proto, uint16_t *Data)
{
  ...
  else if (Proto == IR_MOTOROLA)        /* Motorola */
  {
	...
	// mod by boleslaw_43@mikrocontroller.net
	// The receiver (IR detector) is too slow to receive the second group of bits (with Command), 
    // if the bits are sent according to the Motorola protocol. 
    // It follows, that this tester is not able to check the genuine Motorola remote controls.
	// MilliSleep(20);  // according to Motorola protocol
	#if CPU_FREQ == 8000000
    MilliSleep(118);     
    #endif
	...
	// mod by boleslaw_43@mikrocontroller.net
    IR_Send_Pulse(IR_PULSE, 512);       /* packet start */
    IR_Send_Pulse(IR_PAUSE, 2560);	
	...
	// The receiver (IR detector) is too slow to receive the second group of bits (with Command), 
    // if the bits are sent according to the Motorola protocol. 
    // It follows, that this tester is not able to check the Motorola remote controls.	
