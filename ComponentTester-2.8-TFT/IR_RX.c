...
#if defined (SW_IR_RECEIVER_GRAPHICS) && (defined (FONT_10X16_HF) || defined (FONT_16X26_HF))
uint8_t		    	Join_Char = 0;           /* to reduce space between characters */
#endif
...
void IR_Decode(uint8_t *PulseData, uint8_t Pulses)
{
 ...
 #if defined (SW_IR_RECEIVER_GRAPHICS) && (defined (FONT_10X16_HF) || defined (FONT_16X26_HF))
  uint16_t          Address = 0;       /* RC address */
  uint16_t          Command = 0;       /* RC command */
  uint8_t 			Line;
  uint16_t 			OldColor = UI.PenColor;
#else
  uint8_t           Address = 0;       /* RC address */
  uint8_t           Command = 0;       /* RC command */
#endif
...
#if defined (SW_IR_RECEIVER_GRAPHICS) && (defined (FONT_10X16_HF) || defined (FONT_16X26_HF))
    // Clear screen area, but not first line
  ...
  if (PulseCheck(Time1, 179, IR_RELAX_LONG))      /* pulse 9ms */
  {
    if (PulseCheck(Time2, 89, IR_RELAX_LONG))     /* pause 4.5ms */
    {
...
#if defined (SW_IR_RECEIVER_GRAPHICS) && (defined (FONT_10X16_HF) || defined (FONT_16X26_HF))  
	    Display_NextLine();
		Display_ColoredEEString(IR_NEC_str, COLOR_WHITE);    /* display protocol */
      ...
      else if (Bits == 42)         /* we expect 42 bits for Sanyo */
      {
#if defined (SW_IR_RECEIVER_GRAPHICS) && (defined (FONT_10X16_HF) || defined (FONT_16X26_HF))
	    Display_NextLine();
		Display_ColoredEEString(IR_Sanyo_str, COLOR_WHITE);    /* display protocol */	
	    Display_NextLine();                  /* new line */
		...
		Extras  = GetBits(35, 8, IR_LSB);    /* get key data */
		Command += Extras * 256;
#else
...
#if defined (SW_IR_RECEIVER_GRAPHICS) && (defined (FONT_10X16_HF) || defined (FONT_16X26_HF))
 	      Display_NextLine();
		  Display_ColoredEEString(IR_Proton_str, COLOR_WHITE);    /* display protocol */	
...
#if defined (SW_IR_RECEIVER_GRAPHICS) && (defined (FONT_10X16_HF) || defined (FONT_16X26_HF))
	    Display_NextLine();
		Display_ColoredEEString(IR_JVC_str, COLOR_WHITE);    /* display protocol */
	    Display_Space();           /* display space */		
#else
...
#if defined (SW_IR_RECEIVER_GRAPHICS) && (defined (FONT_10X16_HF) || defined (FONT_16X26_HF))	  
	  Display_ColoredEEString(IR_Matsushita_str, COLOR_WHITE);    /* display protocol */
#else
...
#if defined (SW_IR_RECEIVER_GRAPHICS) && (defined (FONT_10X16_HF) || defined (FONT_16X26_HF))	
	  Display_NextLine();                                       /* new line */
	  Display_ColoredEEString(IR_Kaseikyo_str, COLOR_WHITE);    /* display protocol */	
	  Display_Space();           /* display space */  
	  Display_NextLine();  
#else
...
#if defined (SW_IR_RECEIVER_GRAPHICS) && (defined (FONT_10X16_HF) || defined (FONT_16X26_HF))	  	  
	    Display_NextLine();                                  /* new line */
	    Display_ColoredEEString(IR_RCA_str, COLOR_WHITE);    /* display protocol */	  
	    Display_Space();                                     /* display space */		
#else
      ...
      if (Bits == 10)              /* we expect 10 bits for Motorola */
      {
        ...
        else                            /* command packet */
        {
          /* command packet */
		  	...
		  	Display_NextLine();
 		    Display_ColoredEEString(IR_Motorola_str, COLOR_WHITE);    /* display protocol */
			Display_NextLine();
...
#if defined (SW_IR_RECEIVER_GRAPHICS) && (defined (FONT_10X16_HF) || defined (FONT_16X26_HF))	  	  
	    Display_NextLine();                                      /* new line */
	    Display_ColoredEEString(IR_Thomson_str, COLOR_WHITE);    /* display protocol */	
	    Display_Space();                                         /* display space */	  		
#else
...
#if defined (SW_IR_RECEIVER_GRAPHICS) && (defined (FONT_10X16_HF) || defined (FONT_16X26_HF))	  	  
	    Display_NextLine();                                      /* new line */
	    Display_ColoredEEString(IR_Samsung_str, COLOR_WHITE);    /* display protocol */	
		Display_Space();                                         /* display space */
#else
...
#if defined (SW_IR_RECEIVER_GRAPHICS) && (defined (FONT_10X16_HF) || defined (FONT_16X26_HF))	  	  
	  Display_ColoredEEString(IR_SIRC_str, COLOR_WHITE);  /* display protocol */	
	  Display_NextLine();                                 /* new line */
#else
...
#if defined (SW_IR_RECEIVER_GRAPHICS) && (defined (FONT_10X16_HF) || defined (FONT_16X26_HF))
	      Display_NextLine(); 	  	  
	      Display_ColoredEEString(IR_Sharp_str, COLOR_WHITE);  /* display protocol */	
 	      Display_NextLine();
		  Display_HexByte(Address);        	/* display address */
          Display_Colon();               		/* display: : */
          Display_HexByte(Command);      		/* display command */
...
#if defined (SW_IR_RECEIVER_GRAPHICS) && (defined (FONT_10X16_HF) || defined (FONT_16X26_HF))
	  Display_NextLine(); 	  	  
	  Display_ColoredEEString(IR_RC5_str, COLOR_WHITE);  /* display protocol */	
#else
...
#if defined (SW_IR_RECEIVER_GRAPHICS) && (defined (FONT_10X16_HF) || defined (FONT_16X26_HF))
	  Display_NextLine(); 	  	  
	  Display_ColoredEEString(IR_RC6_str, COLOR_WHITE);  /* display protocol */	
#else
...
result:
  ...
  if (Flag == PACKET_DISPLAY)      /* known protocol & standard output */
  {
    ...
    Join_Char = 0;
	Display_NextLine();            		/* new line */	
	OldColor = UI.PenColor;
	UI.PenColor = COLOR_WHITE;
    Display_EEString(IR_User_str);	
	Display_HexValue(Address,16);    	/* display address */
   	UI.PenColor = OldColor;
	...
	OldColor = UI.PenColor;
	UI.PenColor = COLOR_WHITE;
	Display_EEString(IR_Data_str);	
	Display_HexValue(Command,16);      /* display command */
	UI.PenColor = OldColor;
#else
    Display_HexByte(Address);        	/* display address */
    Display_Colon();               		/* display: : */
...
void IR_Detector(void)
{
  ...
  UI.LineMode = LINE_KEEP;              /* next-line mode: keep first line */
    ...
    MilliSleep(3000);					/* time to set the IR receiver in ZIP socket */
